...
class TestLinguistSourceFileGlobber < MiniTest::Test
  ...
  def mock_ruby_file_found(mocks)
    mocks[:file_blob]
      ...
      .with('a.rb', mocks[:pwd])
      .returns(blob_a_rb)
    mocks[:file_class].expects(:exist?).with('a.rb').returns(true)
    mocks[:file_class].expects(:symlink?).with('a.rb').returns(false)
    mocks[:file_class].expects(:readable?).with('a.rb').returns(true)
  ...
  def mock_markdown_file_found(mocks)
    mocks[:file_blob]
      ...
      .returns(blob_b_md)
    mocks[:file_class].expects(:exist?).with('foo/b.md').returns(true)
    mocks[:file_class].expects(:symlink?).with('foo/b.md').returns(false)
    mocks[:file_class].expects(:readable?).with('foo/b.md').returns(true)
